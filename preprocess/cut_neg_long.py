...
class Negative(Dataset):
  ...
  def __getitem__(self, index):
    ...
    for [net_sta, tp, ts] in picks:
        ...
        out_dir = os.path.join(self.out_root, samp_class, 'negative', date)
...
if __name__ == '__main__':
    ...
    # read fpick
    pick_dict = read_fpick(args.fpick, args.fpha, cut_neg_ratio)
    pick_items = list(pick_dict.items())
    # for dates
    train_paths, valid_paths = [], []
    dataset = Negative(pick_items, args.data_dir, args.out_root)
